...
/// <summary>
/// YOLOv8 OBB (Oriented Bounding Box) model for rotated object detection.
/// Same backbone/neck as detection, head is OBBHead with angle branch.
///
/// Based on official Ultralytics OBB model (yolov8n-obb.pt).
/// </summary>
public class YOLOv8OBBModel : Module<Tensor, OBBHead.OBBOutput>
{
    ...
    // OBB Head
    private readonly OBBHead obbHead;

    public OBBHead Head => obbHead;

    public YOLOv8OBBModel(string name, int nc = 15, string variant = "n",
        int ne = 1, Device? device = null) : base(name)
    {
        NumClasses = nc;
        Variant = variant;
    ...
    public override OBBHead.OBBOutput forward(Tensor x)
    {
        var (fpn_p3, pan_p4, pan_p5) = ForwardBackboneNeck(x);
        return obbHead.forward([fpn_p3, pan_p4, pan_p5]);
    }
    ...
    {
        var (fpn_p3, pan_p4, pan_p5) = ForwardBackboneNeck(x);
        return obbHead.ForwardTrain([fpn_p3, pan_p4, pan_p5]);
    }
}
