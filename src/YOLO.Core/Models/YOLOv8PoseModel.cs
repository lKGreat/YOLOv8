...
/// Shares backbone/neck with detection model, replaces head with PoseHead.
///
/// Matches Python PoseModel(DetectionModel) structure.
/// </summary>
public class YOLOv8PoseModel : Module<Tensor, PoseHead.PoseOutput>
{
    ...
    // Pose Head
    private readonly PoseHead poseHead;

    public PoseHead Head => poseHead;

    public YOLOv8PoseModel(string name, int nc = 1, string variant = "n",
        int nkpt = 17, int ndim = 3, Device? device = null) : base(name)
    {
        NumClasses = nc;
        Variant = variant;
        KptShape = (nkpt, ndim);
    ...
    public override PoseHead.PoseOutput forward(Tensor x)
    {
        var (fpn_p3, pan_p4, pan_p5) = ForwardBackboneNeck(x);
        return poseHead.forward([fpn_p3, pan_p4, pan_p5]);
    }
    ...
    {
        var (fpn_p3, pan_p4, pan_p5) = ForwardBackboneNeck(x);
        return poseHead.ForwardTrain([fpn_p3, pan_p4, pan_p5]);
    }
}
